...
CORE_READ_PATHS = [
    ...
    "/api/love-map/suggestions/shared-future/refinements",
    "/api/blueprint/",
...
class CoreReadAuthorizationMatrixTests(unittest.TestCase):
    ...
    def test_authenticated_can_read_love_map_system(self) -> None:
    ...
    def test_authenticated_can_read_love_map_shared_future_refinements(self) -> None:
        response = self.client.get("/api/love-map/suggestions/shared-future/refinements")
        self.assertEqual(response.status_code, 200)
        self.assertIsInstance(response.json(), list)
