...
SCRIPT_PATH = BACKEND_ROOT / "scripts" / "run_backup_restore_drill_audit.py"
SPEC = importlib.util.spec_from_file_location("run_backup_restore_drill_audit", SCRIPT_PATH)
...
class BackupRestoreDrillAuditTests(unittest.TestCase):
    def test_run_backup_restore_drill_writes_evidence_files(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            payload, json_path, md_path = MODULE.run_backup_restore_drill(
                evidence_dir=Path(tmp_dir),
                now_utc=datetime(2026, 2, 23, 3, 0, tzinfo=UTC),
    ...
    def test_required_checks_are_stable(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            payload, _, _ = MODULE.run_backup_restore_drill(
                evidence_dir=Path(tmp_dir),
                now_utc=datetime(2026, 2, 23, 3, 0, tzinfo=UTC),
                source_evidence_max_age_days=120,
            )
