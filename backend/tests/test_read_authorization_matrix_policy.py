import importlib.util
import sys
import tempfile
import unittest
from pathlib import Path

from fastapi import APIRouter, FastAPI

BACKEND_ROOT = Path(__file__).resolve().parents[1]
if str(BACKEND_ROOT) not in sys.path:
    sys.path.insert(0, str(BACKEND_ROOT))

POLICY_SCRIPT_PATH = BACKEND_ROOT / "scripts" / "check_read_authorization_matrix.py"
_SPEC = importlib.util.spec_from_file_location("check_read_authorization_matrix", POLICY_SCRIPT_PATH)
if _SPEC is None or _SPEC.loader is None:
    raise RuntimeError(f"Unable to load policy module from {POLICY_SCRIPT_PATH}")
_MODULE = importlib.util.module_from_spec(_SPEC)
sys.modules[_SPEC.name] = _MODULE
_SPEC.loader.exec_module(_MODULE)


class ReadAuthorizationMatrixPolicyTests(unittest.TestCase):
    def test_policy_passes_for_current_repository_state(self) -> None:
        violations = _MODULE.collect_read_authorization_matrix_violations()
        self.assertEqual(violations, [])

    def test_policy_rejects_missing_critical_read_route_entry(self) -> None:
        app = FastAPI()
        router = APIRouter()

        @router.get("/api/test/read")
        def read_test_item() -> dict:
            return {"ok": True}

        app.include_router(router)

        matrix_payload = {
            "schema_version": _MODULE.MATRIX_SCHEMA_VERSION,
            "entries": [],
        }
        inventory_payload = {
            "entries": [
                {
                    "protocol": "http",
                    "method": "GET",
                    "path": "/api/test/read",
                    "owner_team": "backend-core",
                }
            ]
        }

        violations = _MODULE.collect_read_authorization_matrix_violations(
            app=app,
            matrix_payload=matrix_payload,
            inventory_payload=inventory_payload,
            critical_read_keys={("GET", "/api/test/read")},
        )

        self.assertTrue(any(v.reason == "missing_matrix_entry" for v in violations))

    def test_policy_rejects_owner_team_mismatch_with_inventory(self) -> None:
        app = FastAPI()
        router = APIRouter()

        @router.get("/api/test/read")
        def read_test_item() -> dict:
            return {"ok": True}

        app.include_router(router)

        matrix_payload = {
            "schema_version": _MODULE.MATRIX_SCHEMA_VERSION,
            "entries": [
                {
                    "method": "GET",
                    "path": "/api/test/read",
                    "auth_mode": "authenticated",
                    "subject_scope": "current_user",
                    "owner_team": "backend-core",
                    "test_ref": "tests/fake_read_authz_file.py",
                }
            ],
        }
        inventory_payload = {
            "entries": [
                {
                    "protocol": "http",
                    "method": "GET",
                    "path": "/api/test/read",
                    "owner_team": "backend-security",
                }
            ]
        }

        with tempfile.TemporaryDirectory() as tmp_dir:
            repo_root = Path(tmp_dir)
            fake_test_ref = repo_root / "tests" / "fake_read_authz_file.py"
            fake_test_ref.parent.mkdir(parents=True, exist_ok=True)
            fake_test_ref.write_text(
                "# READ_AUTHZ_MATRIX: GET /api/test/read\n",
                encoding="utf-8",
            )

            violations = _MODULE.collect_read_authorization_matrix_violations(
                app=app,
                matrix_payload=matrix_payload,
                inventory_payload=inventory_payload,
                repo_root=repo_root,
                critical_read_keys={("GET", "/api/test/read")},
            )

        self.assertTrue(any(v.reason == "owner_team_mismatch" for v in violations))

    def test_policy_rejects_missing_test_ref_marker(self) -> None:
        app = FastAPI()
        router = APIRouter()

        @router.get("/api/test/read")
        def read_test_item() -> dict:
            return {"ok": True}

        app.include_router(router)

        matrix_payload = {
            "schema_version": _MODULE.MATRIX_SCHEMA_VERSION,
            "entries": [
                {
                    "method": "GET",
                    "path": "/api/test/read",
                    "auth_mode": "authenticated",
                    "subject_scope": "current_user",
                    "owner_team": "backend-core",
                    "test_ref": "tests/fake_read_authz_file.py",
                }
            ],
        }
        inventory_payload = {
            "entries": [
                {
                    "protocol": "http",
                    "method": "GET",
                    "path": "/api/test/read",
                    "owner_team": "backend-core",
                }
            ]
        }

        with tempfile.TemporaryDirectory() as tmp_dir:
            repo_root = Path(tmp_dir)
            fake_test_ref = repo_root / "tests" / "fake_read_authz_file.py"
            fake_test_ref.parent.mkdir(parents=True, exist_ok=True)
            fake_test_ref.write_text("# no read authz marker here\n", encoding="utf-8")

            violations = _MODULE.collect_read_authorization_matrix_violations(
                app=app,
                matrix_payload=matrix_payload,
                inventory_payload=inventory_payload,
                repo_root=repo_root,
                critical_read_keys={("GET", "/api/test/read")},
            )

        self.assertTrue(any(v.reason == "missing_test_ref_marker" for v in violations))


if __name__ == "__main__":
    unittest.main()
