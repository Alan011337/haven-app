...
SCRIPT_PATH = BACKEND_ROOT / "scripts" / "run_key_rotation_drill_audit.py"
SPEC = importlib.util.spec_from_file_location("run_key_rotation_drill_audit", SCRIPT_PATH)
...
class KeyRotationDrillAuditTests(unittest.TestCase):
    def test_run_key_rotation_drill_writes_evidence_files(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            payload, json_path, md_path = MODULE.run_key_rotation_drill(
                evidence_dir=Path(tmp_dir),
                now_utc=datetime(2026, 2, 23, 1, 0, tzinfo=UTC),
            )
    ...
    def test_required_checks_are_stable(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            payload, _, _ = MODULE.run_key_rotation_drill(
                evidence_dir=Path(tmp_dir),
                now_utc=datetime(2026, 2, 23, 1, 0, tzinfo=UTC),
            )
