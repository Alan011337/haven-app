...
export default function MediationSkeleton() {
  return (
      ...
      <GlassCard className="overflow-hidden rounded-[3.1rem] border-white/54 bg-white/84 p-6 md:p-8 xl:p-10">
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
          <div className="space-y-6">
            ...
            <div className="h-11 w-44 animate-pulse rounded-full bg-white/76 shadow-soft" aria-hidden />
          </div>

          <div className="space-y-4">
            <div className="h-56 animate-pulse rounded-[2.3rem] bg-white/76 shadow-soft" aria-hidden />
            <div className="h-48 animate-pulse rounded-[2.3rem] bg-white/76 shadow-soft" aria-hidden />
          </div>
        </div>
      </GlassCard>
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
            {Array.from({ length: 5 }).map((_, index) => (
              <div key={index} className="h-28 animate-pulse rounded-[1.7rem] bg-white/76 shadow-soft" aria-hidden />
            ))}
          </div>
        </div>
      </GlassCard>
