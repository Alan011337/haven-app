'use client';

import { GlassCard } from '@/components/haven/GlassCard';

export default function MemorySkeleton() {
  return (
    <div
      className="space-y-[clamp(1.5rem,3vw,2.75rem)] animate-slide-up-fade"
      aria-live="polite"
      aria-busy="true"
    >
      <GlassCard className="overflow-hidden rounded-[3.1rem] border-white/54 bg-white/84 p-6 md:p-8 xl:p-10">
        <div className="grid gap-6 xl:grid-cols-[minmax(0,0.88fr)_minmax(0,1.12fr)]">
          <div className="space-y-6">
            <div className="space-y-4">
              <div className="h-4 w-32 animate-pulse rounded-full bg-muted" aria-hidden />
              <div className="h-16 w-4/5 animate-pulse rounded-[1.6rem] bg-muted" aria-hidden />
              <div className="h-5 w-full animate-pulse rounded-full bg-muted" aria-hidden />
              <div className="h-5 w-4/5 animate-pulse rounded-full bg-muted" aria-hidden />
            </div>

            <div className="h-24 animate-pulse rounded-[1.9rem] bg-white/76 shadow-soft" aria-hidden />

            <div className="grid gap-3 md:grid-cols-3">
              <div className="h-28 animate-pulse rounded-[1.7rem] bg-white/76 shadow-soft" aria-hidden />
              <div className="h-28 animate-pulse rounded-[1.7rem] bg-white/76 shadow-soft" aria-hidden />
              <div className="h-28 animate-pulse rounded-[1.7rem] bg-white/76 shadow-soft" aria-hidden />
            </div>
          </div>

          <div className="grid gap-4 2xl:grid-cols-[minmax(0,1fr)_320px]">
            <div className="h-[28rem] animate-pulse rounded-[2.95rem] bg-white/76 shadow-soft" aria-hidden />
            <div className="space-y-4">
              <div className="h-60 animate-pulse rounded-[2.3rem] bg-white/76 shadow-soft" aria-hidden />
              <div className="h-56 animate-pulse rounded-[2.3rem] bg-white/76 shadow-soft" aria-hidden />
            </div>
          </div>
        </div>
      </GlassCard>

      <GlassCard className="overflow-hidden rounded-[2.45rem] border-white/52 bg-white/82 p-5 md:p-6">
        <div className="space-y-4">
          <div className="space-y-3">
            <div className="h-4 w-24 animate-pulse rounded-full bg-muted" aria-hidden />
            <div className="h-10 w-80 max-w-full animate-pulse rounded-[1.4rem] bg-muted" aria-hidden />
          </div>
          <div className="grid gap-3 md:grid-cols-2">
            <div className="h-36 animate-pulse rounded-[1.85rem] bg-white/76 shadow-soft" aria-hidden />
            <div className="h-36 animate-pulse rounded-[1.85rem] bg-white/76 shadow-soft" aria-hidden />
          </div>
        </div>
      </GlassCard>

      <div className="space-y-4">
        <div className="space-y-3 px-1">
          <div className="h-4 w-24 animate-pulse rounded-full bg-muted" aria-hidden />
          <div className="h-10 w-96 max-w-full animate-pulse rounded-[1.4rem] bg-muted" aria-hidden />
        </div>

        <GlassCard className="overflow-hidden rounded-[2.95rem] border-white/52 bg-white/82 p-6 md:p-8 xl:p-10">
          <div className="space-y-5">
            <div className="flex gap-2">
              <div className="h-8 w-28 animate-pulse rounded-full bg-white/76 shadow-soft" aria-hidden />
              <div className="h-8 w-24 animate-pulse rounded-full bg-white/76 shadow-soft" aria-hidden />
            </div>
            <div className="h-12 w-4/5 animate-pulse rounded-[1.4rem] bg-muted" aria-hidden />
            <div className="h-5 w-full animate-pulse rounded-full bg-muted" aria-hidden />
            <div className="h-5 w-5/6 animate-pulse rounded-full bg-muted" aria-hidden />
            <div className="grid gap-3">
              <div className="h-20 animate-pulse rounded-[1.8rem] bg-white/76 shadow-soft" aria-hidden />
              <div className="h-20 animate-pulse rounded-[1.8rem] bg-white/76 shadow-soft" aria-hidden />
            </div>
          </div>
        </GlassCard>

        <div className="grid gap-4 lg:grid-cols-2">
          <div className="h-72 animate-pulse rounded-[2.3rem] bg-white/76 shadow-soft" aria-hidden />
          <div className="h-72 animate-pulse rounded-[2.3rem] bg-white/76 shadow-soft" aria-hidden />
        </div>

        <div className="grid gap-4">
          {Array.from({ length: 3 }).map((_, index) => (
            <div
              key={index}
              className="h-48 animate-pulse rounded-[2rem] bg-white/76 shadow-soft"
              aria-hidden
            />
          ))}
        </div>
      </div>
    </div>
  );
}
